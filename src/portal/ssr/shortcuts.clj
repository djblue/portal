(ns ^:no-doc portal.ssr.shortcuts
...
(defn- event->key [e]
  (when-let [k (:key e)] (str/lower-case k)))
...
(defn- log->combo
  ...
  (when-let [e (first log)]
    (cond-> #{(event->key e)}
      (:ctrl-key e)  (conj "control")
      (:meta-key e)  (conj "meta")
      (:shift-key e) (conj "shift")
      (:alt-key e)   (conj "alt"))))
...
(defn- init []
  (when (nil? @log)
    (clear!)
    #_(.addEventListener js/window "blur" #(clear!))
    #_(.addEventListener js/window "keydown" #(keydown %))))

(defn matched! [_log]
  (clear!)
  #_(when-let [e (first log)] (.preventDefault ^js e)))
